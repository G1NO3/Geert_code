...
class TicTacToe{
    ...
    private: 
        ...
        std::vector<TicTacToe::State> history;
};
...
TicTacToe::TicTacToe(bool verb){
    verbose = verb;
    reset();
}

/***** State api *****/
//棋盘以100110011001100101的形式储存，每两位代表一个格子，两位的值代表格子上谁下到这里了，如果是0就是unoccupied
...
bool TicTacToe::State::test_win() const {
    ...
    // row
    mask = last_piece | (last_piece << 2) | (last_piece << 4);// put three same pieces in the same row
    for (int i = 0; i < 3; ++ i){
        if ((board & mask) == mask){
            return true;
        }
    ...
    // column;
    mask = last_piece | (last_piece << 6) | (last_piece << 12);// put three same pieces in the same column
    for (int i = 0; i < 3; ++ i){
        if ((board & mask) == mask){
            return true;
        }
...
void TicTacToe::State::print() const {
    for (int y = 0; y < 3; ++ y){
        for (int x = 0; x < 3; ++ x){
            std::cout << "\t" << TicTacToe::PLAYER_NAME[get_piece(TicTacToe::Action(x, y))];//?
        }
        std::cout << std::endl;
    }
