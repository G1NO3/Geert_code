...
class WindyGridWorldPolicySarsa : public WindyGridWorldPolicyBase{
    public:
        ...
        void learn(int iter = 100000){
        	WindyGridWorld env;
        	int action,action_evaluate,x_now,y_now,rewards;
        	double new_q;
        	for (int i=0;i<iter;++i){
        		env.reset();
				action = epsilon_greedy(env.state());
    			x_now = env.state().first;
				y_now = env.state().second;
				rewards = env.step(action).second;
        		while (not env.done()){
					action_evaluate = epsilon_greedy(env.state());
					double& q_sa = q[y_now][x_now][action];
					new_q = q_sa+alpha*(rewards+gamma*q[env.state().second][env.state().first][action_evaluate]-q_sa);
					q_sa = new_q;
        ...
        void learn(int iter = 100000){
            ...
            for (int i=0;i<iter;++i){
            	...
            	while (not env.done()){
            		...
            		y_now = env.state().second;
            		rewards = env.step(action).second;
            		action_evaluate = (*this)(env.state());
            		double& q_sa = q[y_now][x_now][action];
            		new_q = q_sa+alpha*(rewards+gamma*q[env.state().second][env.state().first][action_evaluate]-q_sa);
            		q_sa = new_q;
